...
user_pool_id=$(jq -r '.AWS.Cognito.UserPoolId // empty' "$config_file")
client_id=$(jq -r '.AWS.Cognito.ClientId // empty' "$config_file")
authority=$(jq -r '.AWS.Cognito.Authority // empty' "$config_file")
...
if [ -z "$user_pool_id" ] || [ "$user_pool_id" = "null" ]; then
    echo "ERROR: UserPoolId is missing or empty"
    ((validation_errors++))
fi

if [ -z "$client_id" ] || [ "$client_id" = "null" ]; then
    echo "ERROR: ClientId is missing or empty"
    ((validation_errors++))
fi

# Validate UserPoolId format
if [ -n "$user_pool_id" ] && [[ ! "$user_pool_id" =~ ^[a-z0-9-]+_[a-zA-Z0-9]+$ ]]; then
    echo "ERROR: UserPoolId format is invalid: $user_pool_id"
    ((validation_errors++))
fi

# Validate region consistency
if [ -n "$user_pool_id" ] && [ -n "$region" ]; then
    pool_region=$(echo "$user_pool_id" | cut -d'_' -f1)
    if [ "$pool_region" != "$region" ]; then
        echo "ERROR: UserPoolId region ($pool_region) does not match AWS region ($region)"
        ((validation_errors++))
    fi
fi

# Validate authority URL
if [ -n "$authority" ] && [[ ! "$authority" =~ ^https://cognito-idp\.[a-z0-9-]+\.amazonaws\.com/ ]]; then
    echo "ERROR: Authority URL format is invalid: $authority"
    ((validation_errors++))
fi
...
main() {
    ...
    echo "=================================================="
    if [ $TESTS_FAILED -eq 0 ]; then
        echo -e "${GREEN}✅ All unit tests passed! Cognito integration functionality validated.${NC}"
        exit 0
    else
        echo -e "${RED}❌ $TESTS_FAILED test(s) failed.${NC}"
        echo -e "${YELLOW}📋 Check the test report for details: $TEMP_DIR/cognito_integration_test_report.md${NC}"
        exit 1
    fi
}
