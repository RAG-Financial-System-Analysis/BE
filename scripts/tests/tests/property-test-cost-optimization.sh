...
main() {
    ...
    echo "=================================================="
    if [ $TESTS_FAILED -eq 0 ] && [ ${#PROPERTY_VIOLATIONS[@]} -eq 0 ]; then
        echo -e "${GREEN}✅ All tests passed! Cost optimization constraints property validated.${NC}"
        exit 0
    else
        echo -e "${RED}❌ Some tests failed or property violations detected.${NC}"
        echo -e "${YELLOW}📋 Check the test report for details: $TEMP_DIR/cost_optimization_test_report.md${NC}"
        exit 1
    fi
}
