...
log_test_info() {
    echo -e "${BLUE}[PROPERTY TEST]${NC} $1"
}

log_test_success() {
    echo -e "${GREEN}[PROPERTY PASS]${NC} $1"
    ((TESTS_PASSED++))
}

log_test_failure() {
    echo -e "${RED}[PROPERTY FAIL]${NC} $1"
    ((TESTS_FAILED++))
}
...
main() {
    ...
    echo "=================================================="
    if [ $TESTS_FAILED -eq 0 ] && [ ${#PROPERTY_VIOLATIONS[@]} -eq 0 ]; then
        echo -e "${GREEN}✅ All tests passed! Deployment idempotency property validated.${NC}"
        exit 0
    else
        echo -e "${RED}❌ Some tests failed or property violations detected.${NC}"
        echo -e "${YELLOW}📋 Check the test report for details: $TEMP_DIR/deployment_idempotency_test_report.md${NC}"
        exit 1
    fi
}
