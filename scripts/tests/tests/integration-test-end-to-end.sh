...
main() {
    ...
    echo "=================================================="
    if [ $TESTS_FAILED -eq 0 ]; then
        echo -e "${GREEN}✅ All end-to-end integration tests passed!${NC}"
        echo -e "${GREEN}   Deployment automation system fully validated.${NC}"
        exit 0
    else
        echo -e "${RED}❌ $TESTS_FAILED test scenario(s) failed.${NC}"
        echo -e "${YELLOW}📋 Check the test report for details: $TEMP_DIR/end_to_end_integration_test_report.md${NC}"
        exit 1
    fi
}
