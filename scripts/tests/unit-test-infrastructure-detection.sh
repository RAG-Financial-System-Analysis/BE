...
# Colors for output
readonly INFRA_RED='\033[0;31m'
readonly INFRA_GREEN='\033[0;32m'
readonly INFRA_YELLOW='\033[1;33m'
readonly INFRA_BLUE='\033[0;34m'
readonly INFRA_NC='\033[0m'
...
log_test_info() {
    echo -e "${INFRA_BLUE}[TEST INFO]${INFRA_NC} $1"
}

log_test_success() {
    echo -e "${INFRA_GREEN}[TEST PASS]${INFRA_NC} $1"
    ((TESTS_PASSED++))
}

log_test_failure() {
    echo -e "${INFRA_RED}[TEST FAIL]${INFRA_NC} $1"
    ((TESTS_FAILED++))
}
    ...
    "rds")
        ...
        case "\$subcommand" in
            "describe-db-instances")
                ...
                elif [[ "\$*" == *"--db-instance-identifier nonexistent-test-db"* ]]; then
                    echo '{"Error": {"Code": "DBInstanceNotFoundFault", "Message": "DB instance not found"}}' >&2
                    exit 1
                elif [[ "\$*" == *"--db-instance-identifier"* ]]; then
                    # Any other specific DB identifier that's not myapp-production-db
                    echo '{"Error": {"Code": "DBInstanceNotFoundFault", "Message": "DB instance not found"}}' >&2
                    exit 1
    ...
    "lambda")
        ...
        case "\$subcommand" in
            ...
            "get-function")
                ...
                elif [[ "\$*" == *"--function-name nonexistent-test-api"* ]]; then
                    echo '{"Error": {"Code": "ResourceNotFoundException", "Message": "Function not found"}}' >&2
                    exit 1
                elif [[ "\$*" == *"--function-name"* ]]; then
                    # Any other function name that's not myapp-production-api
                    echo '{"Error": {"Code": "ResourceNotFoundException", "Message": "Function not found"}}' >&2
                    exit 1
                else
...
# Get RDS status - simplified for testing
if $AWS_CLI rds describe-db-instances --db-instance-identifier "$DB_IDENTIFIER" >/dev/null 2>&1; then
    # For testing, we'll assume if the command succeeds, the status is available
...
main() {
    echo -e "${INFRA_BLUE}🧪 Starting $TEST_NAME${INFRA_NC}"
    echo "=================================================="
    ...
    echo "=================================================="
    if [ $TESTS_FAILED -eq 0 ]; then
        echo -e "${INFRA_GREEN}✅ All unit tests passed! Infrastructure detection functionality validated.${INFRA_NC}"
        exit 0
    else
        echo -e "${INFRA_RED}❌ $TESTS_FAILED test(s) failed.${INFRA_NC}"
        echo -e "${INFRA_YELLOW}📋 Check the test report for details: $TEMP_DIR/infrastructure_detection_test_report.md${INFRA_NC}"
        exit 1
    fi
}
