...
<body>
    ...
    <p>{html}</p>
        ...
        public async Task<GetAnalyticsReportsResponse> GetAnalyticsReportsAsync(Guid? sessionId, Guid userId, int page, int pageSize)
        {
            var query = _dbContext.AnalyticsReports.AsQueryable();

            // Filter by user - only show reports generated by this user
            query = query.Where(r => r.Generatedby == userId);
